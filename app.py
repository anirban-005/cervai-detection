...
def download_models_if_needed():
    ...
    def download_from_gdrive(file_id, dest):
        ...
        for key, value in response.cookies.items():
            if key.startswith('download_warning'):
                ...
                break
        if token:
            url = "https://drive.google.com/uc?export=download&confirm=" + token + "&id=" + file_id
            response = session.get(url, stream=True)
    ...
    for name, file_id in models_info.items():
        ...
        try:
            ...
            msg.error(name + ' error: ' + str(e)[:100])
...
def generate_gradcam(image_pil, model_name, trained_models, pred_idx):
    ...
    try:
        ...
        img_exp = np.expand_dims(img_pre, 0)
        grad_model = KModel(inputs=model.inputs, outputs=[model.get_layer(last_conv).output, model.output])
...
def generate_pdf_report(patient_info, prediction, gradcam_img, model_choice):
    ...
    story = []
    title_style = ParagraphStyle('T', parent=styles['Title'], fontSize=22, fontName='Helvetica-Bold', textColor=colors.HexColor('#00d4aa'), alignment=TA_CENTER, spaceAfter=6)
    sub_style = ParagraphStyle('S', parent=styles['Normal'], fontSize=10, textColor=colors.HexColor('#8892a4'), alignment=TA_CENTER, spaceAfter=4)
    sec_style = ParagraphStyle('H', parent=styles['Heading2'], fontSize=13, fontName='Helvetica-Bold', textColor=colors.HexColor('#00d4aa'), spaceBefore=12, spaceAfter=6)
    norm_style = ParagraphStyle('N', parent=styles['Normal'], fontSize=10, textColor=colors.HexColor('#2c3e50'), spaceAfter=4)
    disc_style = ParagraphStyle('D', parent=styles['Normal'], fontSize=8, textColor=colors.HexColor('#999999'), alignment=TA_CENTER)
    ...
    story.append(HRFlowable(width="100%", thickness=2, color=colors.HexColor('#00d4aa'), spaceAfter=12))
    ...
    pt.setStyle(TableStyle([
        ...
        ('FONTSIZE', (0, 0), (-1, -1), 10),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('ROWBACKGROUNDS', (0, 1), (-1, -1), [colors.HexColor('#f8fffe'), colors.HexColor('#e8fdf9')]),
    ...
    risk_colors_map = {'LOW RISK': '#00d4aa', 'MILD RISK': '#ffa733', 'MODERATE RISK': '#ff8c00', 'HIGH RISK': '#ff4b6e', 'CRITICAL RISK': '#ff0000'}
    ...
    pbt.setStyle(TableStyle([
        ...
        ('BACKGROUND', (0, prediction['pred_idx']+1), (-1, prediction['pred_idx']+1), colors.HexColor('#e8fdf9'))
    ]))
    story.append(pbt)
    story.append(Spacer(1, 12))
    if gradcam_img is not None:
        ...
        story.append(Paragraph("Red/warm areas indicate high activation zones influencing the AI prediction.", norm_style))
        ...
        over_buf.seek(0)
        it = Table([
            [RLImage(orig_buf, 5*cm, 5*cm), RLImage(heat_buf, 5*cm, 5*cm), RLImage(over_buf, 5*cm, 5*cm)],
            [Paragraph("Original", norm_style), Paragraph("Heatmap", norm_style), Paragraph("Overlay", norm_style)]
        ])
        it.setStyle(TableStyle([('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('PADDING', (0, 0), (-1, -1), 5)]))
        story.append(it)
        story.append(Spacer(1, 12))
    story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#e0e0e0'), spaceAfter=8))
    ...
    story.append(Spacer(1, 8))
    story.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#e0e0e0'), spaceAfter=8))
    story.append(Paragraph("DISCLAIMER: This AI report is for screening only and does not constitute a medical diagnosis. Please consult a qualified medical professional.", disc_style))
    story.append(Paragraph("CervAI Detection System | Powered by Deep Learning | Report ID: CERVAI-" + datetime.now().strftime('%Y%m%d%H%M%S'), disc_style))
    doc.build(story)
    buffer.seek(0)
    return buffer
...
def send_email(receiver_email, patient_name, prediction, pdf_buffer):
    try:
        ...
        part.add_header('Content-Disposition', 'attachment; filename="CervAI_Report_' + patient_name + '.pdf"')
        msg.attach(part)
...
def send_whatsapp(whatsapp_number, patient_name, prediction):
    try:
        import urllib.parse
        msg_text = "CervAI Medical Report\n\nDear " + patient_name + ",\n\nYour cervical cancer screening report is ready.\n\nRESULTS SUMMARY:\nPatient Name   : " + patient_name + "\nCell Type      : " + prediction['class_disp'] + "\nCancer Stage   : " + prediction['stage'] + "\nRisk Level     : " + prediction['risk'] + "\nConfidence     : " + str(round(prediction['confidence'], 2)) + "%\n\nCLASS PROBABILITIES:\nDyskeratotic             : " + str(round(float(prediction['all_probs'][0])*100, 2)) + "%\nKoilocytotic             : " + str(round(float(prediction['all_probs'][1])*100, 2)) + "%\nMetaplastic              : " + str(round(float(prediction['all_probs'][2])*100, 2)) + "%\nParabasal                : " + str(round(float(prediction['all_probs'][3])*100, 2)) + "%\nSuperficial-Intermediate : " + str(round(float(prediction['all_probs'][4])*100, 2)) + "%\n\nRECOMMENDATION:\n" + get_recommendation(prediction['risk']) + "\n\nReport Date : " + datetime.now().strftime('%d/%m/%Y %I:%M %p') + "\nReport ID   : CERVAI-" + datetime.now().strftime('%Y%m%d%H%M%S') + "\n\nIMPORTANT: AI-assisted screening. Please consult your doctor.\n\nRegards,\nCervAI Detection System"
...
with st.sidebar:
    st.markdown("<div style='text-align:center;padding:1rem 0;'><div style='font-size:3rem;'>🔬</div><div style='font-size:1.3rem;font-weight:700;color:#00d4aa;letter-spacing:2px;'>CervAI</div><div style='font-size:0.75rem;color:#8892a4;letter-spacing:1px;'>DETECTION SYSTEM</div></div>", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("<div style='color:#8892a4;font-size:0.8rem;text-transform:uppercase;letter-spacing:1px;'>Model Selection</div>", unsafe_allow_html=True)
    model_choice = st.radio("Model", ['Ensemble (Recommended)', 'VGG16', 'ResNet50', 'InceptionV3'], label_visibility='collapsed')
    model_key = model_choice.replace(' (Recommended)', '')
    st.markdown("---")
    st.markdown("<div style='color:#8892a4;font-size:0.8rem;text-transform:uppercase;letter-spacing:1px;margin-bottom:0.5rem;'>Model Performance</div>", unsafe_allow_html=True)
    for m, acc in ACCURACY_MAP.items():
        color = '#00d4aa' if m == model_key else '#8892a4'
        bold = 'bold' if m == model_key else 'normal'
        st.markdown("<div style='display:flex;justify-content:space-between;padding:0.3rem 0;color:" + color + ";font-weight:" + bold + ";'><span>" + m + "</span><span>" + str(acc) + "%</span></div>", unsafe_allow_html=True)
    st.markdown("---")
    st.markdown("<div style='color:#8892a4;font-size:0.75rem;text-align:center;line-height:1.6;'>AI-powered cervical cancer<br>detection using Transfer Learning<br>VGG16 + ResNet50 + InceptionV3<br><br><span style='color:#00d4aa;'>v1.0.0</span></div>", unsafe_allow_html=True)


trained_models = load_all_models()

st.markdown("<div style='text-align:center;padding:3rem 2rem 2rem;'><div class='hero-badge'><span class='status-dot'></span>AI System Online</div><div class='hero-title'>CervAI</div><div class='hero-subtitle'>Cervical Cancer Detection and Stage Prediction</div></div>", unsafe_allow_html=True)

col1, col2, col3, col4 = st.columns(4)
for col, val, label, color in [(col1, "96.88%", "Best Accuracy", "#00d4aa"), (col2, "0.9975", "Best AUC Score", "#00a8ff"), (col3, "5", "Cell Classes", "#ffa733"), (col4, "3", "CNN Models", "#ff4b6e")]:
    with col:
        st.markdown("<div class='result-card fade-in'><div class='result-value' style='color:" + color + ";'>" + val + "</div><div class='result-label'>" + label + "</div></div>", unsafe_allow_html=True)
...
with tab1:
    ...
    with col_left:
        ...
        st.markdown("<div style='color:#00d4aa;font-size:0.9rem;margin:-0.5rem 0 0.5rem;'>BMI: <strong>" + str(round(bmi, 1)) + "</strong></div>", unsafe_allow_html=True)
        c5, c6 = st.columns(2)
        with c5:
        ...
        delivery = st.multiselect("Send Via", ['Email', 'WhatsApp'], default=['Email'], label_visibility='collapsed')
            ...
            if not patient_name:
                ...
                with st.spinner("Analyzing image..."):
                    ...
                    if prediction is not None:
                        ...
                        gradcam = generate_gradcam(image_pil, gc_model, trained_models, prediction['pred_idx'])
                if prediction is None:
                    st.error("Prediction failed. Please try again.")
                    st.stop()
                st.markdown("<div class='glass-card fade-in'><div style='text-align:center;margin-bottom:1rem;'><div style='font-size:0.85rem;color:#8892a4;text-transform:uppercase;letter-spacing:2px;'>Predicted Cell Type</div><div style='font-size:2rem;font-weight:900;color:#00d4aa;margin:0.3rem 0;'>" + prediction['class_disp'] + "</div><div style='font-size:0.9rem;color:#8892a4;'>" + prediction['stage'] + "</div></div><div style='text-align:center;margin:1rem 0;'><span class='risk-badge " + prediction['risk_class'] + "'>" + prediction['risk'] + "</span></div><div style='text-align:center;'><div style='font-size:0.8rem;color:#8892a4;text-transform:uppercase;letter-spacing:1px;'>Confidence</div><div style='font-size:2.5rem;font-weight:900;color:" + prediction['risk_color'] + ";font-family:JetBrains Mono,monospace;'>" + str(round(prediction['confidence'], 1)) + "%</div></div></div>", unsafe_allow_html=True)
                st.markdown("<div style='color:#8892a4;font-size:0.8rem;text-transform:uppercase;letter-spacing:1px;margin:1rem 0 0.5rem;'>Class Probabilities</div>", unsafe_allow_html=True)
                for i, (cls_d, prob) in enumerate(zip(CLASS_DISPLAY, prediction['all_probs'])):
                    ...
                    bg = 'linear-gradient(90deg,#00d4aa,#00a8ff)' if i == prediction['pred_idx'] else 'rgba(136,146,164,0.3)'
                    st.markdown("<div style='margin:0.4rem 0;'><div style='display:flex;justify-content:space-between;margin-bottom:3px;'><span style='font-size:0.85rem;color:" + color + ";font-weight:" + fw + ";'>" + cls_d + "</span><span style='font-size:0.85rem;color:" + color + ";font-family:JetBrains Mono,monospace;'>" + str(round(pct, 1)) + "%</span></div><div class='prob-bar-container'><div class='prob-bar' style='width:" + str(pct) + "%;background:" + bg + ";'></div></div></div>", unsafe_allow_html=True)
                if gradcam:
                    st.markdown("<div class='custom-divider'></div>", unsafe_allow_html=True)
                    st.markdown("<div style='color:#8892a4;font-size:0.8rem;text-transform:uppercase;letter-spacing:1px;margin-bottom:0.5rem;'>Grad-CAM Explainability</div>", unsafe_allow_html=True)
                    ...
                    with gc2:
                        ...
                        st.image(cv2.cvtColor(hv, cv2.COLOR_BGR2RGB), caption="Heatmap", use_container_width=True)
                    with gc3:
                        st.image(gradcam[2], caption="Overlay", use_container_width=True)
                patient_info = {'name': patient_name, 'age': patient_age, 'height': patient_height, 'weight': patient_weight, 'bmi': bmi, 'whatsapp': patient_whatsapp, 'email': patient_email}
                pdf_buffer = generate_pdf_report(patient_info, prediction, gradcam, model_key)
                st.markdown("<div class='custom-divider'></div>", unsafe_allow_html=True)
                st.download_button(label="📥 DOWNLOAD PDF REPORT", data=pdf_buffer, file_name="CervAI_Report_" + patient_name + "_" + datetime.now().strftime('%Y%m%d') + ".pdf", mime="application/pdf", use_container_width=True)
                if delivery:
                    st.markdown("<div style='color:#8892a4;font-size:0.8rem;text-transform:uppercase;letter-spacing:1px;margin:0.5rem 0;'>Sending Report...</div>", unsafe_allow_html=True)
                ...
                if 'WhatsApp' in delivery and patient_whatsapp:
                    ...
                    if success:
                        st.markdown("<a href='" + wa_link + "' target='_blank' style='text-decoration:none;'><div style='background:linear-gradient(135deg,#25D366,#128C7E);color:white;padding:1rem 2rem;border-radius:12px;font-size:1rem;font-weight:700;cursor:pointer;width:100%;text-align:center;margin:0.5rem 0;letter-spacing:1px;box-shadow:0 4px 15px rgba(37,211,102,0.3);'>📱 SEND WHATSAPP MESSAGE</div></a><div style='color:#8892a4;font-size:0.8rem;text-align:center;margin-top:0.3rem;'>Click above to open WhatsApp with full report</div>", unsafe_allow_html=True)
                    else:
                        st.warning("WhatsApp error: " + wa_link)
        elif analyze_btn and not uploaded_file:
            st.warning("Please upload a Pap smear image first!")
        else:
            st.markdown("<div class='glass-card' style='text-align:center;padding:4rem 2rem;'><div style='font-size:4rem;margin-bottom:1rem;'>🔬</div><div style='font-size:1.2rem;color:#8892a4;font-weight:300;line-height:1.8;'>Fill in patient details<br>Upload a Pap smear image<br>Click Analyze to get results</div></div>", unsafe_allow_html=True)
...
with tab2:
    ...
    for col, (name, acc) in zip([m1, m2, m3, m4], ACCURACY_MAP.items()):
        with col:
            ...
            st.markdown("<div class='glass-card' style='text-align:center;'><div style='font-size:0.8rem;color:#8892a4;text-transform:uppercase;letter-spacing:1px;'>" + name + "</div><div style='font-size:2.2rem;font-weight:900;color:" + color + ";font-family:JetBrains Mono,monospace;'>" + str(acc) + "%</div><div style='font-size:0.75rem;color:#8892a4;'>Accuracy</div></div>", unsafe_allow_html=True)
    st.markdown("<div class='custom-divider'></div>", unsafe_allow_html=True)
    ...
    for cls_n, cls_d in zip(CLASS_NAMES, CLASS_DISPLAY):
        ...
        st.markdown("<div class='glass-card' style='padding:1rem 1.5rem;margin:0.4rem 0;display:flex;justify-content:space-between;align-items:center;'><div><span style='font-weight:700;color:#e0e6f0;'>" + cls_d + "</span><span style='color:#8892a4;font-size:0.85rem;margin-left:1rem;'>" + stage + "</span></div><span class='risk-badge' style='color:" + color + ";border:1px solid " + color + ";background:rgba(255,255,255,0.05);padding:0.3rem 1rem;font-size:0.8rem;'>" + risk + "</span></div>", unsafe_allow_html=True)

with tab3:
    st.markdown("<div class='glass-card fade-in'><div class='section-header'>About CervAI</div><p style='color:#e0e6f0;line-height:1.8;font-size:1rem;'>CervAI is an AI-powered cervical cancer detection system that uses deep learning and transfer learning to analyze Pap smear images. The system classifies cells into 5 categories and predicts the corresponding cancer stage with high accuracy.</p><div class='section-header' style='margin-top:1.5rem;'>Technology Stack</div><div style='display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-top:0.5rem;'><div style='background:rgba(0,212,170,0.05);border:1px solid rgba(0,212,170,0.2);border-radius:12px;padding:1rem;'><div style='color:#00d4aa;font-weight:700;margin-bottom:0.5rem;'>Models</div><div style='color:#8892a4;font-size:0.9rem;line-height:2;'>VGG16 (96.88%)<br>ResNet50 (95.39%)<br>InceptionV3 (92.93%)<br>Ensemble (Best)</div></div><div style='background:rgba(0,168,255,0.05);border:1px solid rgba(0,168,255,0.2);border-radius:12px;padding:1rem;'><div style='color:#00a8ff;font-weight:700;margin-bottom:0.5rem;'>Features</div><div style='color:#8892a4;font-size:0.9rem;line-height:2;'>Transfer Learning<br>Grad-CAM Explainability<br>PDF Report Generation<br>WhatsApp + Email Delivery</div></div></div><div class='section-header' style='margin-top:1.5rem;'>Disclaimer</div><p style='color:#8892a4;font-size:0.9rem;line-height:1.8;'>This system is intended for screening and research purposes only. It does not replace professional medical diagnosis. Always consult a qualified healthcare professional.</p></div>", unsafe_allow_html=True)